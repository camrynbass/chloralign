...
def processRun(run):
 try:
  with open (OUT + run + '.lite.sam') as currentRunFile:
   print(timestamp() + run + '.lite.sam file detected.')
 except IOError:
  print(timestamp() + 'starting ' + run + ' download...')
  filename = str(run) + 'lite.1'
  filepath = TMP + str(filename)
  command(SRC + 'sratoolkit.3.0.2-centos_linux64/bin/prefetch ' + run + ' -O ' + TMP + run + ' --max-size 420000000000 2> ' + ERR)
  command(SRC + 'sratoolkit.3.0.2-centos_linux64/bin/fasterq-dump --split-files ' + run + ' -O ' + TMP + ' -t ' + TMP + ' -f 2> ' + ERR )
  command(FASTP + ' -h ' + TMP + ' -j ' + TMP + ' -i ' + TMP + run + '_1.fastq -I ' + TMP + run + '_2.fastq -o ' + TMP + run + '_1.trim.fastq -O ' + TMP + run + '_2.trim.fastq 2> ' + ERR)
  command('bwa mem -t 1 ' + SRC + REFGENOME + ' ' + TMP + run + '_1.fastq -I ' + TMP + run + '_2.trim.fastq > ' + TMP + run + '.lite.sam 2> ' + ERR)
  command('samtools view -b -F 1 ' + TMP + run + '.lite.sam -o ' + OUT + run + '.lite.sam' + ' 2> ' + ERR)
  command('rm -rf ' + TMP + run + '*.fastq ' + TMP + run)
 print(timestamp() + run + ' load complete.')
...
def processBiosample(biosample):
  ...
  command(SRC + 'bcftools-1.18/bcftools mpileup -O u -f ' + SRC + REFGENOME + ' ' + TMP + biosample + '.bam | ' + SRC + 'bcftools-1.18/bcftools call -c -O v --ploidy 1 | bcftools view -i \'INFO/DP>10\' -Oz -o ' + TMP + biosample + '.vcf.gz')
